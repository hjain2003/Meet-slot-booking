...
const MeetCardEdit = ({ closeMeetCardEdit, meetId }) => {
    ...
    const saveMeetDetails = async (e) => {
        ...
        try {
            const response = await fetch(`https://meet-slot-booking-backend-hjain2003.vercel.app/meets/editMeet/${meetId}`, {
                method: 'PUT',
                headers: {
    ...
    useEffect(() => {
        const fetchMeetDetails = async () => {
            try {
                const response = await fetch(`https://meet-slot-booking-backend-hjain2003.vercel.app/meets/getMeetbyId/${meetId}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                    }
